...
uint8_t sd_raw_init()
{
   ...
   // we don't have these on recorder machine 
    ...
    IODIR0 |= CS;
    PINSEL0	|= 0x1500;
    S0SPCCR = 150;      //  Set frequency to 400kHz for init
    ...
    S0SPCCR = 8;                  // SCK = 1 MHz, counter > 8 and even
...
{  
    S0SPDR  = b;                   // send next SPI channel 0 data
...
{ 
    S0SPDR  = 0xFF;                   // send next SPI channel 0 data
    while (!(S0SPSR & 0x80)) ;     // wait for transfer completed
    return S0SPDR;
}
